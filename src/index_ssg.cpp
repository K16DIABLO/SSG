...
void IndexSSG::GenerateHashedSet (char* file_name, float* data) {
  uint64_t hash_len = (hash_bitwidth_ >> 3) + 2 * sizeof(float);
...
void IndexSSG::QueryHash (const float* query, uint32_t* hashed_query, const uint64_t num_query) {
...
uint32_t IndexSSG::CandidateSelection (const float query_norm, const uint32_t* hashed_query, std::vector<SimpleNeighbor>& selected_pool, boost::dynamic_bitset<>& flags, const uint32_t* neighbors) {
